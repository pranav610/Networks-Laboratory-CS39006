...
int my_socket(int domain, int type, int protocol)
{
    ...
    // Check 4th parameters in thread creation
...
{   
    ...
    // printf("my_send buff: %s\n", (char*)buf);
    // printf("my_send len: %d\n", len);
    // printf("returning here in mysend\n");
    return len;
}

ssize_t my_recv(int sockfd, void *buf, size_t len, int flags)
{   
    // printf("waiting here in myrecv\n");
    int read;
    pthread_mutex_lock(&mutex_r);
        ...
        // printf("my_recv buff: %s\n", (char*)buf);
        pthread_cond_signal(&cond_r);
    pthread_mutex_unlock(&mutex_r);
    // printf("exiting myrecv\n");
    return read;
}
...
{   
    while(1)
    {   
        // first recieve the size of the message send as 4 byte int
        // printf("R waiting\n");
        pthread_mutex_lock(&mutex_r);
