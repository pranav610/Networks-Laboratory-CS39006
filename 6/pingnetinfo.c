...
int main(int argc, char *argv[])
{
    ...
    for (int ttl = 1; ttl < MAX_HOPS; ttl++)
    {
        ...
        {
            printf("No response from the intermediate destination.\n");
            continue;
        }
        ...
        // printf("Latency Send Recv Stamps\n");
        // for (int i = 0; i < n; i++)
        // {
        //     printf("%d %ld %ld %ld %ld\n", i, sent_stamps[i].tv_sec, sent_stamps[i].tv_usec, recv_stamps[i].tv_sec, recv_stamps[i].tv_usec);
        // }
        // printf("Latency Send Recv Stamps BW\n");
        // for (int i = 0; i < n; i++)
        // {
        //     printf("%d %ld %ld %ld %ld\n", i, sent_stamps_bw[i].tv_sec, sent_stamps_bw[i].tv_usec, recv_stamps_bw[i].tv_sec, recv_stamps_bw[i].tv_usec);
        // }
        ...
        {
            printf("Link %s to %s in not reachable\n");
        ...
        printf("Bandwidth of link %s to %s is %lf MBps\n", prev_ip, inet_ntoa(*(struct in_addr *)&max_ip), bandwidth);
